...
que orientó el proyecto y establece la base sobre la cual se desarrolló el Lean
UX Process. El análisis distingue entre **evidencia secundaria**, que ayuda a
comprender el mercado y el dominio, y **supuestos del equipo** formulados antes
del *Needfinding* documentado posteriormente en el Capítulo 2.
 ...
 *Nota.* La figura representa el recorrido que se utiliza para estudiar dónde puede perderse continuidad de información. No implica que todas las empresas ejecuten exactamente el mismo flujo ni que las fricciones mostradas hayan sido validadas mediante investigación primaria. Elaboración propia.
 ...
 *Nota.* Elaboración propia a partir de *Perú: Estructura Empresarial, 2024* (INEI, 2025b).
 ...
 *Nota.* La figura describe la estructura empresarial nacional y no la composición esperada de clientes de Nexa. Elaboración propia a partir de INEI (2025b).
...
a lo largo de una cadena (GS1, 2017).
...
flowchart TD
    ...
    Y --> C
```
 *Nota.* La figura no implica que Nexa deba implementar todos los estándares o identificadores GS1. Elaboración propia a partir de las dimensiones de contexto empleadas por GS1 en su estándar de trazabilidad.
 ...
 *Nota.* Los autores compararon estas temperaturas con la referencia de no más de 4 °C empleada en el estudio. El caso corresponde a supermercados en México y no representa una medición de clientes o usuarios de Nexa. Adaptado del Cuadro 1 de Arriaga-Lorenzo et al. (2023).
 ...
 *Nota.* La gráfica se utiliza como evidencia contextual de que una condición física relevante puede variar de forma persistente a lo largo de la operación; no establece reglas de aceptación para Nexa. Elaboración propia a partir de los valores publicados por Arriaga-Lorenzo et al. (2023), Cuadro 1.
 ...
 *Nota.* El estudio pertenece al dominio sanitario y no equivale a la distribución comercial atendida por Nexa. Se utiliza únicamente como evidencia peruana de que las desviaciones térmicas pueden aparecer repetidamente cuando una operación depende de cadena de frío. Adaptado de Bravo De la Cruz et al. (2025).
 ...
 *Nota.* La figura contextualiza la recurrencia de desviaciones térmicas; no representa frecuencia esperada en empresas objetivo de Nexa. Elaboración propia a partir de Bravo De la Cruz et al. (2025).
 ...
 *Nota.* Las preguntas son orientaciones de investigación derivadas del contexto físico. No constituyen resultados ni requisitos de interfaz.
...
| --- | --- | --- | --- |
| **INEI (2025b)** | 3 302 973 empresas formales; 96,4 % microempresas; 43,5 % comercio; 5,8 % transporte y almacenamiento. | Nexa se plantea en un entorno empresarial amplio y heterogéneo. | Que todas esas empresas sean clientes potenciales o presenten las mismas fricciones. |
...
| **GS1 (2017)** | CTE, KDE y dimensiones de contexto Who/What/Where/When/Why. | La trazabilidad depende de conservar eventos y contexto. | Que Nexa deba implementar íntegramente GS1. |
 ...
 *Nota.* Esta matriz diferencia contexto externo de evidencia propia del proyecto. Ninguna fuente representa por sí sola validación de producto ni comportamiento validado de usuarios de Nexa. Elaboración propia.

#### Análisis 5W2H del problema

La técnica **5W2H** ordenó la comprensión preliminar antes de formular las
assumptions de Lean UX. El análisis considera **Who, What, Where, When, Why,
How y How Much**, distinguiendo aquello que entonces podía sostenerse desde el
dominio y la evidencia secundaria de aquello que debía investigarse.

*Análisis 5W2H de la problemática*

| Elemento | Análisis aplicado a Nexa | Estado del conocimiento en problem framing |
| --- | --- | --- |
| **Who — ¿A quién afecta?** | Inicialmente se considera al personal que participa en almacén y despacho, a conductores u operadores responsables de la entrega y a compradores B2B que reciben y verifican pedidos. El trabajo comercial y de gestión forma parte del ciclo general, pero no constituye el foco Mobile priorizado principal de esta investigación. | Los actores y responsabilidades se conocían desde el dominio; sus necesidades y comportamientos específicos requerían investigación primaria. |
...
| **How Much — ¿Qué magnitud tiene?** | La magnitud específica para Nexa no está cuantificada. El *Needfinding* cerrado caracteriza tareas y fricciones de la muestra, pero no mide impacto, tiempo ni tamaño de mercado. | **No cuantificado como línea base.** Los datos externos ni las recurrencias de la muestra deben transformarse en porcentajes atribuidos a los usuarios de Nexa. |

 *Nota.* El análisis 5W2H representa el entendimiento disponible antes de la investigación primaria. Su finalidad es orientar qué debe observarse y preguntarse, no convertir supuestos en resultados. Elaboración propia.
 ...
 *Nota.* El mapa resume las dimensiones del 5W2H y no sustituye el detalle de la tabla anterior. Elaboración propia.
...
La intención comercial forma parte del contexto que da origen al pedido, pero la
investigación inicial se concentra en los momentos donde la responsabilidad se
acerca a la operación física: almacén y despacho, ejecución de la entrega y
recepción del comprador. **El problema existe con independencia del dispositivo.**
La estrategia de producto consiste en preservar contexto útil, apropiado para el
rol y trazable entre esas transiciones; Mobile es una hipótesis de canal que debe
contrastarse allí donde el trabajo ocurre junto a la mercadería, durante la
entrega o en el punto de recepción. La movilidad comercial de Sales y otras
funciones administrativas puede formar parte de una evolución posterior, pero no
constituye el núcleo de esta investigación Mobile priorizado.
 ...
 *Nota.* La figura representa una hipótesis causal utilizada para orientar la investigación posterior. No expresa relaciones causalmente demostradas con usuarios de Nexa. Elaboración propia.
 ...
 *Nota.* Los resultados buscados expresan la dirección del problema que se pretende comprender. No representan métricas alcanzadas ni decisiones definitivas de diseño. Elaboración propia.
...
| Tipo | Objetivo |
| --- | --- |
| **Objetivo general** | Analizar y validar las fricciones de continuidad de información que aparecen entre la preparación, el despacho, la entrega y la recepción de pedidos B2B, con el fin de orientar una estrategia de coordinación adecuada para cada rol y, cuando la investigación lo justifique, una experiencia Mobile que ayude a conservar el contexto necesario entre participantes y responsabilidades. |
 ...
 *Nota.* Los objetivos expresan lo que el proyecto debe aprender y validar. Su cumplimiento no se demuestra por la sola construcción de una funcionalidad, sino por la evidencia obtenida durante el proceso. Elaboración propia.

##### Restricciones iniciales

Las restricciones delimitan el alcance de la investigación y evitan convertir
decisiones futuras de producto o tecnología en afirmaciones definitivas sin
...
| --- | --- |
| **Mercado objetivo** | Nexa se orienta a importadores, distribuidores y mayoristas B2B. Los resultados de salud o supermercados se utilizan como contexto y no como representación directa de los futuros usuarios. |
| **Foco Mobile priorizado** | El estudio prioriza almacén y despacho, ejecución de la entrega y recepción del comprador. Una experiencia comercial móvil amplia para Sales y otras funciones de gestión queda fuera del núcleo priorizado y podrá investigarse posteriormente como conveniencia complementaria. |
...
| **Validación** | El *Needfinding* aporta evidencia sobre problemas, tareas y restricciones de la muestra; las fricciones, frecuencias, costos, beneficios y resultados causales de la solución no se presentan como validados. |
 ...
 *Nota.* Las restricciones delimitan el problema y el aprendizaje inicial; no representan una especificación técnica de implementación. Elaboración propia.

La sección 1.2.1 establece así una base deliberadamente preliminar: existía
evidencia suficiente para justificar la investigación de continuidad,
trazabilidad y trabajo móvil dentro del ciclo B2B. El *Needfinding* posterior
aportó evidencia primaria delimitada sobre prácticas y fricciones de la muestra,
sin cuantificar su magnitud ni demostrar qué solución concreta será más
efectiva. Estas preguntas se transformaron en assumptions e hypotheses en el
Lean UX Process de la sección 1.2.2.
