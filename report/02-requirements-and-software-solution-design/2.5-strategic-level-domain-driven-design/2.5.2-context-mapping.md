...
#### Mapa de decisión comercial

*Mapa de contexto: decisiones de compromiso, disponibilidad y crédito.*
...
flowchart LR
  ...
  BC04 -->|demand to reservation/backing| BC05["BC-05 Inventory Availability"]
...
*Nota.* Relaciones TARGET entre contextos dentro de un único Nexa API modular;
no representan servicios, bases de datos ni unidades de despliegue separadas.

BC-02 conserva la elegibilidad, BC-03 resuelve el offer snapshot, BC-05 decide
Inventory Reservation y Warehouse Backing y BC-07 decide Credit Reservation.
BC-04 posee el resultado
...
#### Mapa de ejecución física

*Mapa de contexto: preparación, asignación y entrega física.*
...
*Nota.* BC-05 conserva autoridad sobre stock y asignación; BC-06 ejecuta el
flujo operativo con contratos explícitos.

BC-05 conserva Physical Stock, Sellable Availability, Inventory Reservation,
Warehouse Backing y Physical Allocation;
...
*Mapa de contexto: crédito, pago, obligación y documento.*
...
*Nota.* Payment, Credit & Receivables y Business Documents conservan ciclos de
vida y autoridad propios.
...
#### Mapa de comunicación y trazabilidad

*Mapa de contexto: notificaciones y hechos de negocio durables.*
...
*Nota.* La entrega de comunicación no altera el hecho fuente; Business
Traceability conserva una proyección durable y separada.
...
#### Mapa de alcance Tenant

*Mapa de contexto: aislamiento y autorización con alcance explícito.*
...
*Nota.* La ausencia o ambigüedad de alcance falla cerrada; la autorización de
frontend no sustituye controles de servidor y base de datos.
...
*Relaciones y consistencia entre los once Bounded Contexts aceptados.*
...
| BC-04 Sales Commitment | BC-05 Inventory Availability | Commercial Commitment demand y solicitud de Inventory Reservation. | Síncrona y atómica cuando la protección forma parte de la confirmación. |
| BC-05 Inventory Availability | BC-04 Sales Commitment | Inventory Reservation y Warehouse Backing, con shortage explícito. | Síncrona; no comparte aggregate ni permite escritura directa de stock. |
...
| BC-05 Inventory Availability | BC-06 Fulfillment & Delivery | Physical Allocation y hechos de asignación posteriores a la Reservation y al Warehouse Backing. | Contrato de mutación explícito; la historia física no se reescribe. |
...
*Nota.* Cada relación conserva un contrato explícito; una referencia
intercontexto no autoriza lectura o mutación directa de persistencia ajena.

#### Patrones y frontera transaccional

*Patrones de relación y límites de consistencia del Context Map.*
...
| BC-04/05/07 durante confirmación | Contrato síncrono de consistencia | Commercial Commitment, Inventory Reservation, Warehouse Backing y credit aplicable se deciden dentro de una frontera lógica. |
...
*Nota.* Los patrones describen diseño TARGET. No prueban outbox, inbox,
integraciones ni ejecución de runtime.
...
*Alternativas de frontera evaluadas durante el diseño estratégico.*
...
*Nota.* Rechazar una alternativa de frontera no implica crear microservicios ni
fijar una topología de despliegue.
