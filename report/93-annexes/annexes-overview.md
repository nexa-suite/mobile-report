...
Each annex must begin on a new page and use a capital letter as required by the
course statement. The index below is a controlled register; an annex remains
pending until its source, evidence and reviewer are attached.
...
identify its source, date, scope and permission. The historical captures linked
from the internal ledger are quarantined source-controlled assets and require
review before PDF/publication.

The historical interview captures under Annex A are source-controlled
reference assets. They remain non-authoritative until identity, consent,
content and publication permission are reviewed. `RESEARCH_PENDING` is an
internal queue state, not a research finding.
